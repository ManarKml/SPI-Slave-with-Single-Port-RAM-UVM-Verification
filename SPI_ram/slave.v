...
always@(posedge clk) begin
    ...
    READ_DATA: begin
        ...
        if (tx_valid && counter !=20) begin
            rx_valid <=0;
            MISO <= tx_data[19 - counter]; // output from LSB to MSB
        end
        if (counter == 20) begin flag <=0;
        //MISO <=0; //just prevent MISO from latching the last transmitted bit before the next read operation
        end
    end
    READ_ADD: begin
          if (counter<10) rx_data[9-counter] <= MOSI;
